...
@Component({
  ...
  template: `
    ...
    <div class="table-wrap"><table><thead><tr><th>{{ 'tasks.titleField' | t }}</th><th>{{ 'tasks.assignedTo' | t }}</th><th>{{ 'tasks.lead' | t }}</th><th>{{ 'tasks.dueDate' | t }}</th><th>{{ 'tasks.priority' | t }}</th><th>{{ 'tasks.status' | t }}</th><th>{{ 'common.actions' | t }}</th></tr></thead><tbody>
      @for (task of tasks(); track task.id) {
        <tr><td>{{ task.title }}</td><td>{{ task.assignee?.full_name ?? ('common.unassigned' | t) }}</td><td>{{ task.lead?.name ?? ('common.none' | t) }}</td><td>{{ task.due_date ?? ('common.none' | t) }}</td><td><span class="badge">{{ priorityKey(task.priority) | t }}</span></td><td><span class="badge" [class]="task.status">{{ statusKey(task.status) | t }}</span></td><td><button class="link-button" type="button" (click)="startEdit(task)">{{ 'common.edit' | t }}</button></td></tr>
